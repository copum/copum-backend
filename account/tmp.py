    ...
    def get(self, request):
        try :
            ...
            if User.objects.filter(email=email).exists() :
                ...
                return Response({'message' : "로그인 성공", "token" : access_token, 'error' : False, "user" :{"id": user.id, "user_id": user.user_id, "email": user.email, 'login_type': user.login_type}}, status=200)
            ...
            # # 회원 가입일 경우
                ...
                user.save()
                print(user)
                res =  JsonResponse({'message': "회원가입 성공",
                    "token": {
                        "access" : access_token,
                        "refresh" : refresh_token,
                    },
